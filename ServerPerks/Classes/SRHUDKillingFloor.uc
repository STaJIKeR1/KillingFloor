class SRHUDKillingFloor extends HUD_StoryMode dependson(SRPlayerReplicationInfo);//Flame.
...
//Flame.
var array<SRPlayerReplicationInfo.PlayerData> PlayerDataList;
var array<SRPlayerReplicationInfo.ClanData> ClanDataList;
//
...
simulated function DrawHud(Canvas C)
{
	...
	if ( !KFPlayerReplicationInfo(PlayerOwner.PlayerReplicationInfo).bViewingMatineeCinematic )
	{
		...
		//Flame. Отображаем собственный HUD от 3 лица
		//ScreenPos = C.WorldToScreen(PawnOwner.Location+vect(0,0,1)*PawnOwner.CollisionHeight); //закоментить после проверки
		//DrawPlayerInfo(C, PawnOwner, ScreenPos.X, ScreenPos.Y); //закоментить после проверки
		//
		PassStyle = STY_Alpha;
...
function DrawPlayerInfo(Canvas C, Pawn P, float ScreenLocX, float ScreenLocY)
{
	...
	//Flame.
	SRPRI=SRPlayerReplicationInfo(P.PlayerReplicationInfo);
	if(SRPRI!=none)
		cData = SRPRI.myClanData;
	if(SRPRI!=none && SRPRI.ClanName!="")
	{
		C.SetDrawColor(SRPRI.ColorR, SRPRI.ColorG, SRPRI.ColorB, SRPRI.ColorA);
		ClanName = SRPRI.ClanName;
		C.StrLen(ClanName, sXL, sYL);
		C.SetPos(ScreenLocX - sXL * 0.83 - 10 + cData.clanX, ScreenLocY - sYL * 2.95 + cData.clanY);
	...
	//Flame.
	C.Style = ERenderStyle.STY_Alpha;
	C.SetDrawColor(255, 255, 255, SRPRI.ColorA); //BeaconAlpha
	if(SRPRI.ClanName!="" && cData.ClanTexture!="")
	{
		TempMaterial=Texture(DynamicLoadObject(cData.ClanTexture,class'Texture'));
		if (TempMaterial!=none)
		{
			TempX = ScreenLocX - cData.ClanTextureW - 62 + cData.logoX;
			TempY = ScreenLocY - cData.ClanTextureH/2 - 34 + cData.logoY;
			C.SetPos(TempX, TempY);
